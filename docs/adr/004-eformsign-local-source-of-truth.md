...
- A deleted document stays buried by its `049` status alone: the conditional upsert refuses the update and, because the row exists, raises a stale-update error rather than recreating it. `permanent_purge_requested_at` means something different — "the vendor still owes us this purge" — so the purge clears it, and the reconcile sweep retries the vendor cancellation for any intent that survives.
