...
제공기관 검토(070) 단계에 머무는 산모신생아 계약서를, 서비스(계약) 종료일이 지난 다음날
KST 00:00에 자동으로 검토 완료(headless finalize) 처리한다. 관리자가 수동으로 "검토 완료
확인" wizard를 돌리던 작업의 자동화다.

## 결정 사항 (사용자 확답)

1. **백로그 제외** — 활성화 기준일(`CONTRACT_AUTO_FINALIZE_SINCE`, YYYY-MM-DD) 이전에
   종료된 문서는 영구 제외. 배포 시 그날 날짜를 env에 넣는다.
2. **재시도 총 3회** — 실패 시 다음 자정에 재시도, 3회 소진 시 방치하고 브랜치 사용자
...
- `@Cron("0 0 * * *", { timeZone: "Asia/Seoul" })` + `SchedulerExecutionGuard`.
...
- 후보 선정 (로컬 DB만):
  ...
  - 자격: `SINCE <= endDate < today(KST)` — "9일 종료 → 10일 00:00 처리"는 `endDate < today`로 구현
  - 종료일 추출 실패 → skip + warn
- 실행: 문서당 `FinalizeDocumentHeadlessUsecase.execute({ documentId, prefillEndDate: endDate })`
  **순차** 호출 (Playwright 브라우저 자동화라 병렬 금지). 개별 실패는 기록 후 다음 문서 계속.
  finalize의 벤더 대조 로직(PR #491)이 그대로 적용되므로 무인 실행에서도 거짓 성공 불가.
...
- 자격 판정 순수 함수: 날짜 경계(종료 다음날 포함·당일 미포함), SINCE 경계, 종료일 누락,
  소진 제외.
...
- Railway(3환경)에 `CONTRACT_AUTO_FINALIZE_ENABLED=true`, `CONTRACT_AUTO_FINALIZE_SINCE=<배포일>` 설정.
