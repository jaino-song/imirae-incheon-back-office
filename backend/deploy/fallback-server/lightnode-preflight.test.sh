...
fake sha256sum 'if [[ "$#" == 0 ]]; then /usr/bin/shasum -a 256; else /usr/bin/shasum -a 256 "$1"; fi'
...
fake curl 'case "$*" in *ipify*) printf "%s" "${E1:-$PUBLIC}";; *) printf "%s" "${E2:-$PUBLIC}";; esac'
fake ss 'printf "%b" "${LISTENERS:-tcp LISTEN 0 1 $ANY:22 $ANY:*\nudp UNCONN 0 0 $ANY:41641 $ANY:*}"'
...
ok installed TIMER_OUTPUT=disabled TIMER_STATUS=1 LISTENERS=$"tcp LISTEN 0 1 $ANY:22 $ANY:*\ntcp LISTEN 0 1 $LOOP:3101 $ANY:*\ntcp LISTEN 0 0 [::1]:3102 [::]:*\nudp UNCONN 0 0 $ANY:41641 $ANY:*"
ok installed TIMER_OUTPUT=enabled TIMER_STATUS=0 LISTENERS=$"tcp LISTEN 0 1 $ANY:22 $ANY:*\ntcp LISTEN 0 1 $LOOP:3101 $ANY:*\ntcp LISTEN 0 1 [::1]:3102 [::]:*\nudp UNCONN 0 0 $ANY:41641 $ANY:*"
no installed TIMER_OUTPUT=disabled TIMER_STATUS=0; no installed TIMER_OUTPUT=enabled TIMER_STATUS=1
