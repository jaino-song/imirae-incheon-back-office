...
hash(){ "$(command -v node)" -e 'const fs=require("fs"),c=require("crypto");process.stdout.write(c.createHash("sha256").update(fs.readFileSync(process.argv[1])).digest("hex"))' "$1"; }
...
fake df '[[ "${DF_BAD:-}" == 1 ]] && { echo bad; exit; }; printf "Filesystem 1024-blocks Used Available Capacity Mounted on\n/dev/vda %s 1 %s 1%% /\n" "${TOTAL:-43945313}" "${FREE:-20971520}"'
...
no fresh DF_BAD=1; no fresh SYSTEMD=degraded; no fresh DOCKER_FAIL='info '; no fresh DOCKER_FAIL='compose version'; no fresh NETWORKS=$'bridge\nhost\nother'
...
# The production guard is source-verifiable and fixture-only: no test runs a host check without test mode.
grep -Fq 'test_mode() { [[ "${LIGHTNODE_PREFLIGHT_TEST_MODE:-}" == 1 ]]; }' "$SCRIPT"
...
printf 'operator.sh=%s\ncompose.yml=%s\ncompose.temporary-active.yml=%s\nproduction-db-identity.sh=%s\nsystemd/babyjamjam-fallback-temporary-active-guard.service=%s\nsystemd/babyjamjam-fallback-temporary-active-guard.timer=%s\n' "$(hash "$TMP/o")" "$(hash "$TMP/a/compose.yml")" "$(hash "$TMP/a/compose.temporary-active.yml")" "$(hash "$TMP/a/production-db-identity.sh")" "$(hash "$TMP/u/babyjamjam-fallback-temporary-active-guard.service")" "$(hash "$TMP/u/babyjamjam-fallback-temporary-active-guard.timer")" >"$TMP/a/bundle.manifest"; chmod 640 "$TMP/a/bundle.manifest"
...
grep -Fq 'controller_residue' "$SCRIPT"
