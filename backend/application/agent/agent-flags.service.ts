...
export class AgentFlagsService {
    private cache: { raw: AgentFlagsConfig; expiresAt: number } | null = null;
    ...
    async getConfig(): Promise<AgentFlagsConfig> {
        return this.applyEnvironmentKillSwitches(await this.getRawConfig());
    }
    ...
    private async getRawConfig(): Promise<AgentFlagsConfig> {
        const now = Date.now();
        ...
        this.cache = { raw: value, expiresAt: now + CACHE_TTL_MS };
        return value;
    }
