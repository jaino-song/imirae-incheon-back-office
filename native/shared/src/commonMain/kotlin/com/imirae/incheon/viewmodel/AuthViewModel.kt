...
import com.imirae.incheon.auth.LogoutState
...
class AuthViewModel(private val authManager: AuthManager) {
    ...
    val logoutState: StateFlow<LogoutState> = authManager.logoutState
    ...
    fun register(name: String, email: String, password: String, phone: String, birthDate: String) {
        ...
        if (!Validation.validateKoreanPhoneNumber(phone).isValid) return
        if (!Validation.validateBirthDate(birthDate).isValid) return
        authManager.register(name, email, password, phone, birthDate)
    }
    fun logout() = authManager.logout()
    fun restoreSession() = authManager.restoreSession()
    fun onAppResume() = authManager.onAppResume()
