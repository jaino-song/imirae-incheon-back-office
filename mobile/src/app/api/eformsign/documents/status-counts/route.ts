...
const PASSTHROUGH_FILTER_PARAMS = ["templateId", "templateMatch", "search"] as const;

/**
 * GET /api/eformsign/documents/status-counts
 * Raw status signals for the current branch, with the same pre-slice filters as
 * the paginated documents endpoint (templateId/templateMatch/search/excludeDeleted).
 * The client folds the signals into per-category counts for the filter pills.
 */
